...
def I2INetFC(x, nfilters=32, init='xavier', activation=tf.nn.relu, output_filters=1):
    ...
    sp = fullyConnected(y_vec,CROP_DIMS,activation, std=init, scope='sp1',reuse=tf.AUTO_REUSE)
    sp = fullyConnected(y_vec,CROP_DIMS**2,activation, std=init, scope='sp2',reuse=tf.AUTO_REUSE)
